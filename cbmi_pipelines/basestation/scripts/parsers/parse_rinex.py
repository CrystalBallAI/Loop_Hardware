#!/usr/bin/env python3
"""parse_rinex.py — SRC_BASE_RINEX parser.

Emits L1F_BASE_001..017 (17 source fields) for the RINEX observation file.

Single streaming pass over the body to keep memory bounded for multi-hour
sessions (~150 MB / 26 k epochs in the sample). The pass produces:
  - per-satellite C/N0 mean and std-dev (Welford accumulator)
  - per-satellite cycle-slip counts (LLI bit 0)
  - epoch timestamps used to derive obs_start/end, epoch_interval_sec,
    inter-epoch gap stats (feeds L2D gap/continuity fields)
  - sat_count_per_epoch summary plus first-window samples (feeds acquisition)

Hardware Override (4-tier priority) for marker/antenna/receiver:
  1. RINEX header (skipped if blank)
  2. sample_data/hardware.json  (operator override)
...
PARSER_VERSION = "1.2"  # 1.2: header parsing via georinex (hybrid); body streamer unchanged
SOURCE_FILE_ID = "SRC_BASE_RINEX"

# Cadence for PDOP samples (seconds of session time). PDOP varies slowly with
# satellite geometry; 30s sampling over a multi-hour session captures the
# mean/max profile with negligible loss.
PDOP_SAMPLE_INTERVAL_SEC = 30.0
PDOP_ELEVATION_MASK_DEG = 10.0  # survey-GNSS industry standard (RTKLIB / TBC / Leica default)

# RINEX versions accepted by typical PPK pipelines (drives format_score / FLG_006).
...
# C/N0 sample window for acquisition fallback: keep all epochs in the first
# 600 s so the acquisition derivation has full resolution.
...
def _read_lines_text(path: Path):
...
def _parse_header(path: Path) -> dict[str, Any]:
    ...
    Body streaming below stays in-house for speed; only header parsing was
    swapped to the library after benchmarking confirmed the body load was
    600× slower under the full xarray load path.
    """
    ...
    # string for spec compatibility (L2D_BASE_013 looks up against a string set).
    ver = raw.get("version")
    ...
    header: dict[str, Any] = {
        ...
        "_lines_read": None,            # georinex doesn't expose this
...
def _infer_from_comments(field_name: str, comments: list[str]) -> str:
    ...
    # receiver firmware. Do not infer — Tier 4 (honest empty) is more correct.
    return ""


def _load_override(project_root: Path) -> dict | None:
    candidate = project_root / "sample_data" / "hardware.json"
...
def _compute_pdop_series(
    ...
    receiver position is unknown, returns a structured null result.
    """
    result: dict[str, Any] = {
        "samples": [],
    ...
    # Lazy import to keep numpy out of the import path when PDOP is unused.
    import sys as _sys
    _sys.path.insert(0, str(Path(__file__).resolve().parent))
    try:
        from cbmi_pipelines.basestation.scripts.parsers import parse_nav  # type: ignore
        from cbmi_pipelines.basestation.scripts.parsers import gnss_orbits  # type: ignore
    finally:
        _sys.path.pop(0)
    ...
    if skipped_no_eph > 0:
        result["notes"].append(
            ...
            "broadcast NAV doesn't cover every observed satellite — e.g. Galileo records "
            "absent in this NAV file)."
        )
    result["notes"].append(
...
def parse(rinex_obs_path: Path, project_root: Path) -> dict[str, Any]:
    ...
    # 4-tier resolved hardware fields
    ...
    if not header["_end_of_header_seen"]:
        notes.append("END OF HEADER record not seen — header parse may be incomplete.")
    ...
    if header["antenna_delta_h_e_n"] == [0.0, 0.0, 0.0]:
        notes.append(
            "antenna_delta_h is 0.000 in RINEX header — typical when device unset. "
            "L2D_BASE_018 antenna_height_agreement will skip the cross-check (per spec rule: only when delta_h != 0)."
        )
    ...
    if pdop_result["summary"]["n_samples"] > 0:
        ...
        notes.append(
            f"L1F_BASE_016 pdop_per_epoch computed from broadcast ephemeris "
        ...
        for n in pdop_result["notes"]:
            notes.append(f"L1F_BASE_016 pdop_per_epoch null: {n}")
    notes.append(
        "L1F_BASE_014 cn0_per_sat retained as per-satellite Welford aggregates (mean, std, n, per band) "
        "rather than full per-epoch arrays. Multipath proxy (L2D_BASE_009) and overall mean "
        "(L2D_BASE_008) are computable from these aggregates."
    )
    notes.append(
        "L1F_BASE_017 sat_count_per_epoch retained as overall summary + first "
        f"{int(ACQUISITION_WINDOW_SEC)}s samples for acquisition derivation (L2D_BASE_012)."
    )

    rinex_version_str = header["rinex_version"]
    if rinex_version_str and rinex_version_str not in SUPPORTED_RINEX_VERSIONS:
        notes.append(f"RINEX version {rinex_version_str} not in supported set; will trip BASE_RINEX_VERSION_UNSUPPORTED at L3I_BASE_003.")

    obs_start_utc_iso = _iso(obs_start_dt) if obs_start_dt else None
    obs_end_utc_iso = _iso(obs_end_dt) if obs_end_dt else None

    fields = {
        "L1F_BASE_001_marker_name": marker_name,
        "L1F_BASE_002_antenna_type": antenna_type,
        "L1F_BASE_003_antenna_delta_h": (
            header["antenna_delta_h_e_n"][0] if header["antenna_delta_h_e_n"] else None
        ),
        "L1F_BASE_004_receiver_type": receiver_type,
        "L1F_BASE_005_firmware_version": firmware_version,
        "L1F_BASE_006_approx_position_xyz": header["approx_position_xyz"],
        "L1F_BASE_007_rinex_version": rinex_version_str,
        "L1F_BASE_008_constellation_set": constellation_set,
        "L1F_BASE_009_obs_start_utc": obs_start_utc_iso,
        "L1F_BASE_010_obs_end_utc": obs_end_utc_iso,
        "L1F_BASE_011_dual_freq_present": dual_freq_present,
        "L1F_BASE_012_epoch_interval_sec": (
            round(epoch_interval_sec, 4) if epoch_interval_sec is not None else None
        ),
        "L1F_BASE_013_total_epochs": body["epoch_count"],
    ...
    field_sources = {
        "L1F_BASE_001_marker_name": marker_src,
        "L1F_BASE_002_antenna_type": antenna_type_src,
        "L1F_BASE_003_antenna_delta_h": "tier_1_rinex_header" if header["antenna_delta_h_e_n"] else "tier_4_empty_with_note",
        "L1F_BASE_004_receiver_type": receiver_type_src,
        "L1F_BASE_005_firmware_version": firmware_src,
        "L1F_BASE_006_approx_position_xyz": "tier_1_rinex_header" if header["approx_position_xyz"] else "tier_4_empty_with_note",
        "L1F_BASE_007_rinex_version": "tier_1_rinex_header",
        "L1F_BASE_008_constellation_set": "tier_1_rinex_header_sys_obs_types",
        "L1F_BASE_009_obs_start_utc": "body_first_epoch",
        "L1F_BASE_010_obs_end_utc": "body_last_epoch",
        "L1F_BASE_011_dual_freq_present": "tier_1_rinex_header_sys_obs_types",
        "L1F_BASE_012_epoch_interval_sec": (
            "tier_1_rinex_header_interval" if header["interval_header_sec"] else "body_median_inter_epoch"
        ),
        "L1F_BASE_013_total_epochs": "body_stream_count",
        "L1F_BASE_014_cn0_per_sat": "body_stream_per_sat_welford",
        "L1F_BASE_015_cycle_slip_markers": "body_stream_lli_bit0",
        "L1F_BASE_016_pdop_per_epoch": (
            "computed_from_broadcast_ephemeris"
            if pdop_result["summary"]["n_samples"] > 0
            else "null_no_nav_or_no_receiver_pos"
        ),
        "L1F_BASE_017_sat_count_per_epoch": "body_stream_epoch_nsat",
    }
    ...
    parser_meta = {
        ...
        "rinex_header": {
            ...
            "_lines_read_during_header": header["_lines_read"],
        ...
        "hardware_override_path": "sample_data/hardware.json" if override else None,
        "notes": notes,
