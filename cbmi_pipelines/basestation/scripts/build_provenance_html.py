#!/usr/bin/env python3
"""Generate a single-file HTML reference for the Base Station (PPK) provenance bundle.

Reads `base_station_confidence_score/base_station_confidence_score.json` (canonical) and writes
`base_station_confidence_score/base_station_provenance.html` — a browser-navigable mirror of the
xlsx with sticky nav, per-table search filter, deep-link ID anchors, and severity + stage badges.
Self-contained: inline CSS + tiny inline JS; no network dependencies.

Ported from the Check Point generator with Base Station divergences (D3):
  1. IDs/counts/prefixes are BASE (SRC_BASE_/L1F_BASE_/L2D_BASE_/L3I_BASE_/BB_BASE_/FLG_BASE_).
     Brand "Base Station Provenance (PPK)"; title "base_station_confidence_score v2.0.0".
  2. Apex (single-record, no per-point logic) — `base_station_score` has score_id, display_name,
     workflow, phase, formula_expression, global_gate_condition, global_gate_action, plus
     scope_note + source_file_set. It has NO per_point_formula, NO aggregator, NO null_handling
     (those are per-point only). Block-weight key is `weight_in_base_station_score`; weights
     list is `base_station_score_blocks` (each entry has key `weight`).
  3. Stage badges — adds `.stg-handoff` (teal) and `.stg-handoff-crossdoc` (slate) on top of
     the existing `stg-composite / stg-global-gate / stg-internal-gate / stg-threshold`.
  4. Severities include `CATASTROPHIC` (problems) plus CRITICAL/HIGH/MEDIUM (flags); CSS
     defines sev-catastrophic explicitly (sev-low / sev-advisory kept unused for future-proof).
  5. Indicators table has NO `data_availability_by_device_type` column (CheckPoint-only).
  6. Building Blocks table renders `formula` (instead of CheckPoint's aggregator +
     indicators_within which BaseStation doesn't carry).
  7. Output file: `base_station_provenance.html`. The pre-existing ad-hoc has been backed up
     out-of-band to `base_station_provenance_v1_adhoc.html` before regeneration.
...
SPEC_PATH = ROOT / "base_station_confidence_score" / "base_station_confidence_score.json"
OUT_PATH = ROOT / "base_station_confidence_score" / "base_station_provenance.html"
...
def severity_badge(sev):
    """Severity slug from the FIRST WORD so compound labels (CheckPoint-style) still produce
    valid CSS classes. BaseStation severities are single words today, but this is harmless
    and future-proofs the helper."""
    if not sev:
        return ""
...
def render_source_files(spec):
    ...
    return section("source-files", f"01 Source Files ({len(spec['source_files'])})",
                   "Physical sources from which fields are extracted (RINEX OBS+NAV / Operation Log / Antenna Setup Record).", body)
...
def render_source_fields(spec):
    ...
    return section("source-fields", f"02 Source Fields ({len(spec['source_fields'])})",
                   "L1F_BASE_* — extracted by Stage 2 parsers (one occupation per survey).", body)
...
def render_derived_fields(spec):
    ...
    return section("derived-fields", f"03 Derived Fields ({len(spec['derived_fields'])})",
                   "L2D_BASE_* — computed in Stage 3a; kinds = scoring / composite_flag / handoff.", body)
...
def render_indicators(spec):
    ...
    for i in spec["indicators"]:
        ...
        rows.append(row([
            ...
            esc(i.get("gate_action") or ""), esc(i.get("threshold_summary")), esc(i.get("justification")),
        ]))
    body = table_head(["indicator_id", "indicator_name", "display_name", "building_block_id",
                       ...
                       "gate_action", "threshold_summary (bands)", "justification"]) + table_body(rows)
    return section("indicators", f"04 Indicators ({len(spec['indicators'])})",
                   "L3I_BASE_* — scored in Stage 3b; band ladder is in threshold_summary (Option B).", body)
...
def render_building_blocks(spec):
    ...
    for b in spec["building_blocks"]:
        ...
        rows.append(row([
            ...
            f'<strong>{esc(b.get("weight_in_base_station_score"))}</strong>', esc(b.get("question")),
            esc(b.get("failure_owner")), esc(b.get("operator_action")), code(b.get("formula")),
            gate + esc(b.get("gate_condition") or ""), esc(b.get("gate_action") or ""),
        ]))
    body = table_head(["block_id", "block_name", "display_name", "weight_in_base_station_score", "question",
                       "failure_owner", "operator_action", "formula",
                       "internal_gate", "gate_action"]) + table_body(rows)
    return section("building-blocks", f"05 Building Blocks ({len(spec['building_blocks'])})",
                   "BB_BASE_* — weighted sum of contained indicators in Stage 3c.", body)


def render_score(spec):
    g = spec["base_station_score"]
    # Single-record apex — no per_point_formula / aggregator / null_handling.
    kv = [(k, g.get(k)) for k in ["score_id", "display_name", "workflow", "phase",
          "formula_expression", "global_gate_condition", "global_gate_action"]]
    wrows = [row([ref(w["block_id"]), esc(w.get("block_name")), f'<strong>{esc(w.get("weight"))}</strong>'])
             for w in spec.get("base_station_score_blocks", [])]
    weights = f'<table class="data-table">{table_head(["block_id", "block_name", "weight"])}{table_body(wrows)}</table>'
    inner = (f'<h3 class="sub-h3">Apex scoring</h3>{kv_table(kv, slug="base-station-score")}'
             f'<h3 class="sub-h3">Apex block weights (sum = 1.0)</h3>{weights}')
    return section_kv("base-station-score", "06 Base Station Score",
                      "Apex formula, global gate, and block weights (single-record — one base occupation per survey).", inner)


def render_score_meta(spec):
    g = spec["base_station_score"]
    # BaseStation 06b is just scope_note + source_file_set (no device_types_supported,
    # no known_limitations — those are CheckPoint-only).
    kv = [(k, g.get(k)) for k in ["scope_note", "source_file_set"]]
    return section_kv("score-meta", "06b Score Meta", "Apex scope and the source-file set this chain reads.",
...
def render_flags(spec):
    ...
    return section("flags", f"07 Flags ({len(spec['flags'])})",
                   "FLG_BASE_* — grouped by raised_at_stage. Severity + stage badges. "
                   "Stages include composite, internal_gate, threshold, global_gate, handoff, handoff_crossdoc.", body)
...
def render_problem_coverage(spec):
    ...
    return section("problem-coverage", f"08 Problem Coverage ({len(spec['problem_coverage_map'])})",
                   "CBMI Base Station problems and how this subsystem (or a sibling) covers each.", body)
...
NAV_LINKS = [
    ...
    ("building-blocks", "05 Building Blocks"), ("base-station-score", "06 Base Station Score"),
...
def nav():
    ...
    return f'<nav class="topnav"><div class="brand">Base Station Provenance (PPK)</div><div class="nav-links">{items}</div></nav>'
...
.stg-handoff{background:#a7f3d0;color:#065f46;}
.stg-handoff-crossdoc{background:#cbd5e1;color:#334155;}
...
def main():
    ...
    title = f"{meta.get('title', 'base_station_confidence_score')} v{meta.get('version', '')}"
    out = f"""<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<footer style="text-align:center;padding:20px 0;color:#64748b;font-size:12px;">
  Generated from <code>base_station_confidence_score.json</code> v{html.escape(str(meta.get('version', '')))}
  &middot; This HTML mirrors <code>base_station_confidence_score.xlsx</code>; the JSON is the canonical source.
</footer>
<script>{JS}</script>
</body>
</html>
