...
def _import_parsers():
    ...
    from cbmi_pipelines.basestation.scripts.parsers import parse_rinex  # type: ignore
    from cbmi_pipelines.basestation.scripts.parsers import parse_oplog  # type: ignore
    from cbmi_pipelines.basestation.scripts.parsers import parse_user_input  # type: ignore
...
def run(config: dict[str, Any], project_root: Path, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    rinex_folder = project_root / inputs["rinex_folder"]
    oplog_folder = project_root / inputs["operator_log_folder"]
    form_folder = project_root / inputs["user_input_folder"]
