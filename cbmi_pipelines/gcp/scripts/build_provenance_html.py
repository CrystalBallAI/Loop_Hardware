#!/usr/bin/env python3
"""Generate a single-file HTML reference for the GCP PPK provenance bundle.

Reads `gcp_confidence_score/gcp_confidence_score.json` (canonical) and writes
`gcp_confidence_score/gcp_provenance.html` — a browser-navigable mirror of the xlsx with:

- Sticky top nav linking to every section
- Per-table search filter (no external JS deps)
- ID anchors (#L3I_GCP_001 / #BB_GCP_COMPLETE / #FLG_GCP_001 / #problem-8 ...) for deep-linking
- Color-coded severity + stage badges on the Flags sheet
- _meta + design summary + audit counts at the top
- Self-contained: inline CSS + tiny inline JS; no network dependencies

GCP differs from the drone reference: bands live in indicator.threshold_summary (no separate
Thresholds sheet), there is no BlockComposition / ScriptHints sheet, the apex is split across
06 (GcpScore) + 06b (ScoreMeta), and there is an 08 Problem Coverage section.
...
SPEC_PATH = ROOT / "gcp_confidence_score" / "gcp_confidence_score.json"
OUT_PATH = ROOT / "gcp_confidence_score" / "gcp_provenance.html"
...
def severity_badge(sev):
    if not sev:
        return ""
    return f'<span class="badge sev-{html.escape(sev.lower())}">{html.escape(sev)}</span>'
...
def render_source_files(spec):
    ...
    return section("source-files", f"01 Source Files ({len(spec['source_files'])})",
                   "Physical sources from which per-point fields are extracted.", body)
...
def render_source_fields(spec):
    ...
    return section("source-fields", f"02 Source Fields ({len(spec['source_fields'])})",
                   "L1F_GCP_* — extracted by Stage 2 parsers (per point).", body)
...
def render_derived_fields(spec):
    ...
    return section("derived-fields", f"03 Derived Fields ({len(spec['derived_fields'])})",
                   "L2D_GCP_* — computed per point in Stage 3a.", body)


def render_indicators(spec):
    rows = []
    for i in spec["indicators"]:
        gate = '<span class="badge gate">internal gate</span> ' if str(i.get("has_internal_gate")).upper() in ("TRUE", "YES") or i.get("has_internal_gate") is True else ""
        rows.append(row([
    ...
    return section("indicators", f"04 Indicators ({len(spec['indicators'])})",
                   "L3I_GCP_* — scored in Stage 3b; band ladder is in threshold_summary (Option B).", body)


def render_building_blocks(spec):
    rows = []
    for b in spec["building_blocks"]:
        gate = '<span class="badge gate">internal gate</span> ' if b.get("has_internal_gate") in (True, "TRUE", "YES") else ""
        rows.append(row([
            id_anchor(b["block_id"]), esc(b["block_name"]), esc(b.get("display_name")),
            f'<strong>{esc(b.get("weight_in_gcp_score"))}</strong>', esc(b.get("question")),
    ...
    body = table_head(["block_id", "block_name", "display_name", "weight_in_gcp_score", "question",
    ...
    return section("building-blocks", f"05 Building Blocks ({len(spec['building_blocks'])})",
                   "BB_GCP_* — rolled up per point then cross-point aggregated in Stage 3c.", body)


def render_gcp_score(spec):
    g = spec["gcp_score"]
    ...
    wrows = [row([ref(w["block_id"]), esc(w.get("block_name")), f'<strong>{esc(w.get("weight"))}</strong>'])
             for w in spec.get("gcp_score_blocks", [])]
    weights = f'<table class="data-table">{table_head(["block_id", "block_name", "weight"])}{table_body(wrows)}</table>'
    inner = (f'<h3 class="sub-h3">Apex scoring</h3>{kv_table(kv, slug="gcp-score")}'
             f'<h3 class="sub-h3">Apex block weights (sum = 1.0)</h3>{weights}')
    return section_kv("gcp-score", "06 GCP Score", "Apex formula, per-point formula, gates, and block weights.", inner)
...
def render_flags(spec):
    ...
    return section("flags", f"07 Flags ({len(spec['flags'])})",
                   "FLG_GCP_* — grouped by raised_at_stage. Severity + stage badges.", body)
...
def render_problem_coverage(spec):
    ...
    return section("problem-coverage", f"08 Problem Coverage ({len(spec['problem_coverage_map'])})",
                   "CBMI GCP problems and how this subsystem (or a sibling) covers each.", body)
...
NAV_LINKS = [
    ...
    ("building-blocks", "05 Building Blocks"), ("gcp-score", "06 GCP Score"),
...
def nav():
    ...
    return f'<nav class="topnav"><div class="brand">GCP Provenance PPK</div><div class="nav-links">{items}</div></nav>'
...
def main():
    ...
    sheets_html = "\n".join([
        ...
        render_gcp_score(spec), render_score_meta(spec), render_flags(spec), render_problem_coverage(spec),
    ])
    title = f"{meta.get('title', 'gcp_confidence_score')} v{meta.get('version', '')}"
    out = f"""<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<footer style="text-align:center;padding:20px 0;color:#64748b;font-size:12px;">
  Generated from <code>gcp_confidence_score.json</code> v{html.escape(str(meta.get('version', '')))}
  &middot; This HTML mirrors <code>gcp_confidence_score.xlsx</code>; the JSON is the canonical source.
</footer>
<script>{JS}</script>
</body>
</html>
